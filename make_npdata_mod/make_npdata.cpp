...
int validate_npd_hashes(const char* file_name, unsigned char *klicensee, NPD_HEADER *npd, bool verbose)
{
	...
	title_hash_result = cmac_hash_compare(NPDRM_OMAC_KEY_3, 0x10, buf, 0x30 + (int)file_name_length, npd->title_hash, 0x10);
...
int encrypt_data(FILE *in, FILE *out, EDAT_HEADER *edat, NPD_HEADER *npd, unsigned char* crypt_key, bool verbose)
{
	...
	for (i = 0; i < block_num; i++)
	{
		...
		else
		{
			...
			{
				if (verbose)
					printf("WARNING: Block at offset 0x%08llx got invalid forged hash!\n", offset);
			}
		}
...
void forge_npd_title_hash(const char* file_name, NPD_HEADER *npd)
{
	...
	cmac_hash_forge(NPDRM_OMAC_KEY_3, 0x10, buf, 0x30 + (int)file_name_length, title_hash);
...
#if __STATIC_LIB
int make_npdata(int argc, char** argv)
#else
	...
	else if (!strcmp(argv[arg_offset + 1], "-d") && (argc > (arg_offset + 4)))
	{
		...
		case 9:
		{
			...
			if (argv[arg_offset + 4] != NULL)
			{
				if (is_hex(argv[arg_offset + 4], 0x20))
				{
					unsigned char custom_rif[0x10];
					hex_to_bytes(custom_rif, argv[arg_offset + 4], 0x20);
					memcpy(rifkey, custom_rif, 0x10);
					arg_offset++;
