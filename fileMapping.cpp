...
FileMapping * fileMappingCreate(LPCWSTR fname) {
  HANDLE hFile = CreateFile(fname, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
  ...
  DWORD dwFileSize = GetFileSizeEx(hFile, &filesize);
  ...
  HANDLE hMapping = CreateFileMapping(hFile, nullptr, PAGE_READWRITE, filesize.HighPart, filesize.LowPart, nullptr);
  ...
  unsigned char* dataPtr = (unsigned char*)MapViewOfFile(hMapping, FILE_MAP_ALL_ACCESS, 0, 0, size);
  if(dataPtr == nullptr) {
