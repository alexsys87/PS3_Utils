...
#include "ldr_curves.h"
#include "vsh_curves.h"
...
void _print_key_list(FILE *fp)
{
	...
	LIST_FOREACH(iter, _keysets)
		if((tmp = (u32)strlen(((keyset_t *)iter->value)->name)) > len)
			len = tmp;
	...
	LIST_FOREACH(iter, _keysets)
	{
		...
		_print_align(fp, " ", len, (u32)strlen(ks->name));
...
BOOL curves_load_embeded(void)
{
	_curves = (curve_t*)malloc(ldr_curvesSize);
	memcpy(_curves, ldr_curves, ldr_curvesSize);
...
BOOL vsh_curves_load_embeded(void)
{
	_vsh_curves = (vsh_curve_t*)malloc(vsh_curvesSize);
	memcpy(_vsh_curves, vsh_curves, vsh_curvesSize);
