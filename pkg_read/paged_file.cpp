...
{
  f->fd = fd;
  f->ptr = (u8*)malloc (PAGED_FILE_PAGE_SIZE);
  f->size = 0;
  f->pos = 0;
  f->page_pos = 0;
  f->reader = reader;
...
paged_file_hash_internal (PagedFile *f)
{
  if (f->hash && (f->size - f->pos) > 0)
    HMACInput (&f->hmac_ctx, f->ptr + f->pos, (u32)(f->size - f->pos));
}
...
paged_file_crypt_internal_encrypt (PagedFile *f)
{
  if (f->size > 0) {
    if (f->crypt == PAGED_FILE_CRYPT_AES_128_CBC) {
      ...
      f->crypt_cb (f, PAGED_FILE_CRYPT_ENCRYPT, f->ptr, (u32)f->size,
          f->crypt_cb_data);
    }
  }
}
  ...
  if (f->size - f->pos > 0) {
    if (f->crypt == PAGED_FILE_CRYPT_AES_128_CBC) {
      ...
      f->crypt_cb (f, PAGED_FILE_CRYPT_DECRYPT, f->ptr + f->pos,
          (u32)(f->size - f->pos), f->crypt_cb_data);
    }
  }
}
...
u64
paged_file_read (PagedFile *f, void *buffer, u64 len)
{
  u64 ret;
  u64 pos = 0;
  u64 size = len;
  ...
  while (len > 0) {
    ...
    memcpy ((u8*)buffer + pos, f->ptr + f->pos, (size_t)size);
    f->pos += size;
    pos += size;
    len -= size;
  }
...
paged_file_flush (PagedFile *f)
{
  ...
  written = fwrite (f->ptr, 1, (size_t)f->size, f->fd);
  f->page_pos += f->size;
  f->size = 0;
...
u64
paged_file_write (PagedFile *f, void *buffer, u64 len)
{
  u64 ret;
  u64 pos = 0;
  u64 size = len;
  ...
  while (len > 0) {
    ...
    memcpy (f->ptr + f->pos, (u8 *)buffer + pos, (size_t)size);
    f->pos += size;
    if (f->pos > f->size)
      f->size = f->pos;
    pos += size;
...
paged_file_seek (PagedFile *f, u64 offset)
{
  ...
  if (f->crypt != PAGED_FILE_CRYPT_NONE) {
    ...
    if (f->crypt == PAGED_FILE_CRYPT_AES_128_CBC ||
      ...
      if (offset >= 0x10) {
        _fseeki64 (f->fd, offset - 0x10, SEEK_SET);
        if (fread (f->iv, 1, 0x10, f->fd) != 1)
          return -1;
      }
    } else if (f->crypt == PAGED_FILE_CRYPT_AES_128_CTR) {
      s64 seek_diff = (s64)(offset - (f->page_pos + f->size)) / 0x10;
      ...
      if (!f->crypt_cb (f, PAGED_FILE_CRYPT_SEEK,
              NULL, (u32)offset, f->crypt_cb_data))
        return -1;
    }
  }
  _fseeki64 (f->fd, offset, SEEK_SET);
  f->size = 0;
  f->page_pos = _ftelli64 (f->fd);
