...
u8 key[0x10] = { 0x2e, 0x7b, 0x71, 0xd7, 0xc9, 0xc9, 0xa1, 0x4e, 0xa3, 0x22, 0x1f, 0x18, 0x88, 0x28, 0xb8, 0xf8 };
...
pkg_debug_decrypt (PagedFile *f, PagedFileCryptOperation operation,
    u8 *ptr, u64 len, void *user_data)
{
  u64 *crypt_offset = (u64*)user_data;
  u8 key[0x40];
  u8 bfr[0x14];
  u64 i;
  s64 seek;
  ...
  if (operation == PAGED_FILE_CRYPT_DECRYPT ||
    ...
    seek = (s64) ((f->page_pos + f->pos) - *crypt_offset) / 0x10;
    if (seek > 0)
    ...
    PKG_HEADER *header, PKG_FILE_HEADER **files)
{
  ...
  *files = (PKG_FILE_HEADER*)malloc (header->item_count * sizeof(PKG_FILE_HEADER));
...
pkg_list (const char *filename)
{
  ...
  for (i = 0; i < header.item_count; i++) {
    ...
    pkg_file_path = (char*)malloc (files[i].filename_size + 1);
